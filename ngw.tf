...
resource "aws_nat_gateway" "eks_nat_ngw_1a" {
...
resource "aws_nat_gateway" "eks_nat_ngw_1b" {
...
resource "aws_route_table" "eks_private_route_table_1a" {
  ...
  route {
    ...
    nat_gateway_id = aws_nat_gateway.eks_nat_ngw_1a.id
  }
...
resource "aws_route_table" "eks_private_route_table_1b" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.eks_nat_ngw_1b.id
  }
